...
- (void)drawRect:(CGRect)rect {
    ...
    [NSObject gl_drawTextBackGroundInRect:rect content:ctx boderColor:COCColorBorder boderWidth:SCALE_Length(1.0f) fillColor:[UIColor clearColor]];
}
...
- (void)setIsCanEdit:(BOOL)isCanEdit {
    ...
    if (_isCanEdit) {
        self.textField.backgroundColor = COCColorBackGround;
    }else {
        self.textField.backgroundColor = COCColorSeparator;
    }
...
- (void)p_setUpUI {
    ...
    self.backgroundColor = COCColorBackGround;
    if (!self.isHideFT) {
...
- (IndentTextField *)textField {
    if (!_textField) {
        ...
        _textField.backgroundColor = COCColorBackGround;
